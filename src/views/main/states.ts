...
import { Dir, kvParse, memParse, unwrap } from "@/rust";
    ...
    async function handleParseFileByPath(): Promise<boolean> {
        ...
        switch (backendMode.value) {
            case ParseBackend.Mem:
                const memResult = unwrap(await memParse(fileSelectionForm.value.path));
                if (memResult) {
            ...
            case ParseBackend.Sled:
                const sledResult = unwrap(await kvParse(fileSelectionForm.value.path));
                if (sledResult) {
                    var end = new Date().getTime()
                    window.$message?.success(`解析完成: ${end - start}ms`)
